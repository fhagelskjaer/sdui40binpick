...
def main():
    ...
    output_name = "grasp_pose_obj_wrs_21.pickle" 
    model_name_def = "../data/21_MBRAC60-2-10.stl"

    object_model = o3d.io.read_triangle_mesh(model_name_def)

    grasp_offset = [0, 0, -4.0]
    finger_opening = 14
    orient_object = compute_obj_orientation(model_name_def, center_offset=[0, 0, -8], rotation_info=['x', 90])
    ...
    grasp_offset = [0, 0, 20]
    finger_opening = 7
    orient_object = compute_obj_orientation(model_name_def, center_offset=[0, 0, 5], rotation_info=['x', 90])
